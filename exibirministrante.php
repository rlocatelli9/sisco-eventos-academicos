...
//CABE�ALHO E RODAP�          
    ...
    class PDF extends FPDF {
        ...
        function Header() {
            ...
            $this->Cell(0, 5, 'Ficha individual de Ministrante', 0, 0, 'C', 0); #Título do Relat�rio
...
//CONTEUDO DO PDF
    ...
    $query_atividade = "SELECT funcao_ministrante.descricao_fministrante, pessoa.nome, ministrantes.idministrantes, ministrantes.descricao_resumida, "
            . "atividades.titulo FROM ministrantes "
...
//cabeçalho da tabela 
    ...
    while ($resultado2 = mysql_fetch_array($exe_query)) {
        ...
        $pdf->Cell(30, 7, "Função:", 0, 0, 'L', 0);
        $pdf->setFont('arial', '', 12);
        $pdf->Cell(0, 7, utf8_encode($resultado2['descricao_fministrante']), 0, 1, 'L', 0);

//email
        $pdf->SetFont('arial', 'B', 12);
        $pdf->Cell(30, 7, "Nome:", 0, 0, 'L', 0);
        $pdf->setFont('arial', '', 12);
        $pdf->Cell(0, 7, utf8_encode($resultado2['nome']), 0, 1, 'L', 0);

//cep
        $pdf->SetFont('arial', 'B', 12);
        $pdf->Cell(30, 7, "Atividade:", 0, 0, 'L', 0);
        $pdf->setFont('arial', '', 12);
        $pdf->Cell(0, 7, utf8_encode($resultado2['titulo']), 0, 1, 'L', 0);
        $pdf->Ln(8); #Quebra de Linha  
//Endere�o
        $pdf->SetFont('arial', 'B', 12);
        $pdf->Cell(90, 7, "Observações sobre o ministrante:", 0, 1, 'L', 0);
        $pdf->setFont('arial', '', 12);
        $pdf->Cell(0, 7, utf8_encode($resultado2['descricao_resumida']), 0, 1, 'J', 0);
    }
