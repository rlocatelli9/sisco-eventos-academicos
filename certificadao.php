...
//$sql = mysql_query("SELECT * FROM pessoa WHERE idpessoa=$id;") or die (mysql_error());
    $sql = mysql_query("SELECT participacao.idparticipante, participacao.Pessoa_idpessoa, pessoa.nome, atividades.idatividades, atividades.titulo, atividades.cargahoraria, tipo_atividade.descricao, evento.periodoon, evento.idevento, pessoa.idpessoa "
    ...
    while ($resultado = mysql_fetch_array($sql)) {        
        ...
        $pdf->Ln(30); #Quebra de Linhas
        $pdf->SetX(40); #Trava o cursor para escrever no Ponto Y        
        $pdf->MultiCell(240, 10, 'Certificamos que ' . $resultado['nome'] . ', Participou do(a) ' . $resultado['descricao'] . ' ' . utf8_encode($resultado['titulo']) . ', no periodo de ' . $resultado['periodoon'] . '.', 0, 'L', 0, 0);
        $pdf->Ln(1);
        ...
        $idparticipacao=$resultado['idparticipante'];
        ...
        require ("conexao.php"); // A MINHA CONEX�O FICOU EM CONFIG.PHP
        $codigo = mysql_query("SELECT participacao_idparticipacao, participacao_atividades_idatividades, participacao_atividades_evento_idevento, participacao_Pessoa_idpessoa, codigo_validacao "
                . "FROM certificado WHERE codigo_validacao = '$variavel' AND participacao_Pessoa_idpessoa = $id;") or die(mysql_error());
        if ($codigo) {
            if (mysql_num_rows($codigo) == 0) {
                $insert = mysql_query("INSERT INTO certificado (participacao_idparticipacao, participacao_atividades_idatividades, "
                        . "participacao_atividades_evento_idevento, participacao_Pessoa_idpessoa, codigo_validacao) "
                        . "VALUES ($idparticipacao, $idatividade, $idevento, $idpessoa, '$variavel');") or die(mysql_error());
              }
        }
    }
