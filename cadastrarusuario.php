                    ...
                    <div class="container-fluid">
                        <div class="row-fluid">
                            ...
                            <div class="span9">
                                ...
                                //verificando email e cpf e recolhendo valor
                                $email = filter_input(INPUT_POST, 'Email');
                                $cpf = filter_input(INPUT_POST, 'Cpf');
                                //$dado2 = filter_input(INPUT_POST, 'Cpf');
                                if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                                    echo '<script>alert("Email não é válido!")</script>';
                                    echo '<body onLoad="document.cadastro.Email.focus();">';
                                    echo '<script>history.back();</script>';
                                } else {
                                    # Verificando apenas um campo, no caso dado1.
                                    $sql = $conect->query("SELECT * FROM pessoa WHERE cpf='$cpf';") or die(mysql_error());
                                    $sql2 = $conect->query("SELECT * FROM pessoa WHERE email='$email';") or die(mysql_error());
                                }
                                if (mysqli_num_rows($sql) > 0) {
                                    echo '<script>alert("O CPF não está disponível para cadastro!")</script>';
                                    echo "<script>window.location.href='administrador.php'</script>";
                                } elseif(mysqli_num_rows($sql2) > 0) {
                                    echo '<script>alert("O Email não está disponível para cadastro!")</script>';
                                    ...
                                    VALUES ('" . utf8_decode(filter_input(INPUT_POST, 'Nome')) . "',
                                            ...
                                            '" . utf8_decode(filter_input(INPUT_POST, 'Origem')) . "',
...
//    Limpa os destinat�rios e os anexos
                                    ...
                                    echo "<script>alert('Não foi possível capturar os dados. Tente novamente mais tarde.')</script>";
                                    echo "<script>window.location.href='index.php'</script>";
