...
$query_pessoa = "SELECT nome, email, Nome_Instituicao FROM pessoa WHERE ativo=1;";
...
while ($resultado2 = mysql_fetch_array($exe_query)) {
    $pdf->Cell(60, 10, utf8_encode($resultado2['nome']), 1, 0, "L");
    $pdf->Cell(60, 10, $resultado2['email'], 1, 0, "L");
    $pdf->Cell(60, 10, utf8_encode($resultado2['Nome_Instituicao']), 1, 0, "L");
    //$pdf->Cell(60, 10, $resultado2['datanasc_formatada'], 1, 0, "C");
    $pdf->Ln();
}
$pdf->Cell(60, 10, '', 0, 0, "C");
$pdf->Cell(60, 10, '', 0, 0, "C");
$pdf->Cell(60, 10,'número de cadastros ativos: ' . $num_rows, 0, 0, "C");
ob_end_clean(); #al�m de limpar tamb�m mostra aquilo que foi armazenado no buffer                
$pdf->Output("usuarios.pdf","D"); #necess�rio para gerar o PDF. Dentro dos parentes voc� consegue definir se o PDF ser� salvo e o nome com caminho de onde ser� salvo
